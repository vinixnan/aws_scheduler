...
def get_aws_regions_full():
    return ['US East (N. Virginia)', 'Europe (Stockholm)', 'US East (Ohio)', 'US West (N. California)', 'US West (Oregon)', 'Africa (Cape Town)', 'Asia Pacific (Hong Kong)', 'Asia Pacific (Mumbai)', 'Asia Pacific (Osaka-Local)', 'Asia Pacific (Seoul)', 'Asia Pacific (Singapore)', 'Asia Pacific (Sydney)', 'Asia Pacific (Tokyo)', 'Canada (Central)', 'Europe (Frankfurt)', 'Europe (Ireland)', 'Europe (London)', 'Europe (Milan)', 'Europe (Paris)', 'Europe (Stockholm)', 'Middle East (Bahrain)', 'South America (Sao Paulo)']
...
def generate_aws_dict(regions):
    ...
    for region, val in dccv.items():
        ...
        for new_id, (_, item) in enumerate(sorted_data, start=0):
            item['id'] = new_id
