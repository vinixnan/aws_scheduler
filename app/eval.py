...
def get_simple_decision(task2ins, ins2type, taskInOrder, task_names, machines_dataset):
    ...
    for id_task in range (len(ins2type)):
        ...
        dc_types[instance] = Machine('host'+str(instance), machines_dataset[machine_type], 'link'+str(instance))
...
problems=["Montage_25", "Montage_50", "Montage_100", "Epigenomics_24",
...
for problem in problems:
    ...
    for iteration in range(max_iter):
        ...
        for line in Lines:
            ...
            data['pysim_price'] = math.ceil(float(resp["makespan"]) / 3600) * sum([machine.data["pricePerUnit"] for machine in machines.values()])
